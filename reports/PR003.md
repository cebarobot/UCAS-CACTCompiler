...
对于源代码中的 `if-else`、`while` 控制流，其涉及到多处跳转操作，需要添加多处标号，以 `if-else` 为例，如表达式 `if (B) S1 else S2`，需要生成两个标号 `B.true` 和 `B.false`，其中 `B.true` 需要置于 `S1` 前，`B.false` 需要置于 `S2` 前，此外，还需要在  `S2` 前设置标号 `S.next`，使得 `S1` 执行后能够跳过 `S2`， 其中 `S` 为次表达式整体。由此思路可以看出，为实现 `if-else` 控制流，可以在 `stmt` 中添加一个前置变量，用于存储对应的跳转标号。
...
在此过程中我们将标号生成后直接插入到对应的前置变量中即可。而对于 `while` 控制流的处理流程，其方案和 `if-else` 相比只是在因此一次执行结束后新增了一个可能的循环，因此直接在代码结尾加上循环语句即可，具体实现如下：
